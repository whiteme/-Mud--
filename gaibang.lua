...
function gaibang:shield()
end

function gaibang:catch_place()
  wait.make(function()
    local l,w=wait.regexp("^(> |)吴长老说道：「此人现在在(.*)一带，务必在.*之前赶回来。.*$",5)
	if l==nil then
	  self:ask_job()
	  return
	end
	if string.find(l,"此人现在在") then
	  self.place=w[2]
...
function gaibang:catch_badman()
  wait.make(function()
    local l,w=wait.regexp("^(> |)吴长老说道：「好吧，最近「(.*)」一直和我丐帮作对，你前去设法将此人除掉！」$|^(> |)吴长老说道：「现在我这里没有给你的任务，你去其他地方看看吧？」$|^(> |)吴长老说道：「现在我可没有给你的任务，等会再来吧！」$|吴长老说道：「你连上个任务去都不想去，是想找近的吧？等会再来！」",5)
	if l==nil then
	  self:ask_job()
	  return
	end
...
function gaibang:ask_job()
  ...
  w.walkover=function()
   wait.make(function()
    ...
    world.Send("ask wu zhanglao about job")
    local l,w=wait.regexp("^(> |)你向吴长老打听有关『job』的消息。$",5)
	if l==nil then
	  self:ask_job()
	  return
	end
	if string.find(l,"你向吴长老打听有关") then
	  self:catch_badman()
...
function gaibang:reward()
  ...
  w.walkover=function()
   wait.make(function()
    world.Send("give shouji to wu zhanglao")

    local l,w=wait.regexp("^(> |)你给吴长老一颗.*的首级。$|^(> |)你身上没有这样东西。$|^(> |)吴长老说道：「我好象没有给过你任务啊？」$",5)
	if l==nil then
	  self:reward()
	  return
	end
	if string.find(l,"你给吴长老一颗") then
	  --self:exps()
	  --[[local rc=reward.new()
	  rc.finish=function()
...
function gaibang:combat()

end

function gaibang:qie_corpse(index)
local b=busy.new()
b.interval=0.5
b.Next=function()
  wait.make(function()
...
function gaibang:auto_wield_weapon(f,error_deal)
...
--[[
function gaibang:test(place,name)
  ...
  local ts={
   ...
   if name=="江北大盗" then
	...
	if n>0 then
	 ...
	 b.Next=function()
	   self:shield()
	   gaibang.co=coroutine.create(function()
...
function gaibang:jobDone()
end

function gaibang:giveup()
 gaibang.co=nil
 local w=walk.new()
  w.walkover=function()
   wait.make(function()
    world.Send("ask wu zhanglao about 放弃")
    local l,w=wait.regexp("^(> |)你向吴长老打听有关『放弃』的消息。$",5)
	if l==nil then
	  self:giveup()
	  return
	end
...
function gaibang:NextPoint()
   print("进程恢复")
   coroutine.resume(gaibang.co)
end

function gaibang:checkPlace(npc,r,here)
...
function gaibang:badman_die()
end

function gaibang:wait_badman_die()
wait.make(function()
...
function gaibang:look_id(npc,id)
  world.Send("look "..id)
...
function gaibang:check_auto_kill_npc(npc,id)
   wait.make(function()
     world.Send("look")
	 world.Send("set look 1")
	 world.Send("unset wimpy")
...
function gaibang:checkNPC(npc,roomno)
    wait.make(function()
