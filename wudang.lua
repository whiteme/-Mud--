...
wudang.__index=wudang

local combat_start_time=nil
local search_start_time=nil

function wudang:NextPoint()
   print("进程恢复")
   coroutine.resume(wudang.co)
end

function wudang:Child_checkRobber(npc,roomno)
...
function wudang:Child_NextPoint()
   print("子进程恢复")
   coroutine.resume(wudang.co2)
end
...
function wudang:quick_checkRobber(alias_txt)
  wait.make(function()
      marco(alias_txt)
      local l,w=wait.regexp("^(> |)设定环境变量：action \\= \\\"逆转\\\"",6)
	  if l==nil then
...
function wudang:auto_pfm()
   print("提前pfm")
end

function wudang:Child_Search(rooms,npc)
-- 丝绸之路 排除掉
  ...
  local location=world.GetVariable("robber_place")
   ...
   if P=="华山村" or P=="兰州城大道" or P=="嵩山山路" or P=="杭州城山路" or P=="回疆" then
      is_reverse_ok=false  --不满足可逆条件
   end
   ...
   if n>1 and is_reverse_ok==true then
	  ...
	  local g=reverse_path(s)  --south;west;west;east;east;east;east;north;east;east;west
	  if g~=false then
		 ...
		 alias_txt=alias_txt.."set action 逆转"
		 print("auto kill 启动")
		 self:auto_kill()
		 self:quick_checkRobber(alias_txt)
		 coroutine.yield()
...
function wudang:Check_Point_Robber()
 --渡船
	 local target_room={}
...
function wudang:check_next_point(target_room)
      wudang.co2=coroutine.create(function()
		 -- 回到主进程上去
...
function wudang:combat()

end

function wudang:run(i)
...
function wudang:sure_robber()
end

function wudang:get_id(npc)
	wait.make(function()
		 world.Execute("look")
		 self:look()
...
--你对着司徒蒋大喝一声：「老匹夫！我就是人贱人爱！贱~~贱~~魔无敌！！(-。-)y-゜゜゜"！」
   wait.make(function()
	  ...
	  if string.find(l,"这里地方太小") then
	       ...
	       self:checkRobber(self.robber_name,callback)
	      --[[shutdown()
...
--这里没有这个人。
--你对着芙蓉喝道：「贱人！你死期已到，今天就让大爷我送你上西天吧！」
function wudang:kill_robber(id)
...
function wudang:checkRobber(npc,CallBack)
...
function wudang:checkPlace(roomno,here,is_omit)
       print("起始房间:",roomno)
      ...
      if is_contain(roomno,here) or is_omit==true then
		   ...
		   b.Next=function()
             ...
             for _,g in ipairs(ex_rooms) do
			  ...
			  al.zishanlin_zishanlin2_quick=function()
			     al:zishanlin_search()
			  end
			  ---
		    al.songlin_check=function()
			...
			al.zishanlin_check=function()
			   world.ColourNote("blue","yellow","**************************搜索紫杉林***********************")
	          local f2=function()
...
function wudang:Search(paths,rooms,npc)
    self.round=1
...
function wudang:c_paths(e,depth,omit,opentime)
  depth=depth+4
...
function wudang:robber(location,depth,npc)
	local ts={
	           task_name="武当",
	           task_stepname="找强盗",
	           task_step=3,
  ...
  --shamo_lvzhou
   ...
   if string.find(location,"平定州") then
      omit="shamo_lvzhou|duhe|dutan|R6154"
   end
   ...
   print("房间数目:",n)
    ...
    if n>0 then
	 ...
	 b.Next=function()
	   ...
	   self:shield()
	   print("抓取")
	   wudang.co=coroutine.create(function()
...
function wudang:robber_die() --跑了
end

function wudang:roober_die2()  --死了
end

function wudang:robber_finish()

--了沈大喊一声：不好！！转身几个起落就不见了。
--白瑛「啪」的一声倒在地上，挣扎着抽动了几下就死了。婷霞大喊一声：不好！！转身几个起落就不见了。
--乐徐「啪」的一声倒在地上，挣扎着抽动了几下就死了。
   wait.make(function()
      local l,w=wait.regexp("^(> |)(.*)大喊一声：不好！！转身几个起落就不见了。|^(> |)(.*)「啪」的一声倒在地上，挣扎着抽动.*|^(> |)(.*)神志迷糊，脚下一个不稳，倒在地上昏了过去。$|^(> |)对手已经这样了，不用兴师动众了.*",10)
	  if l==nil then
	     self:robber_finish()
		 return
	  end
	  ...
	  if string.find(l,"倒在地上昏了过去") or string.find(l,"对手已经这样了") then
	     local name=w[6]
		 print(name)
		 if string.find(self.robber_name,name) then
			 world.Send("kill "..self.robber_id)
		 end
		 self:robber_finish()
		 return
	  end
...
function wudang:deal_blacklist(skill,party)
...
function wudang:redure_difficulty()
...
function wudang:select_difficulty(strong)
...
function wudang:npc_strong()

   wait.make(function()
      local l,w=wait.regexp("^(> |)宋远桥在你的耳边悄声说道：此人的武功(.*)，.*$|^(> |)设定环境变量：ask \\= \\\"YES\\\"$",5)
	  if l==nil then
	     self:npc_strong()
		 return
	  end
	  if string.find(l,"宋远桥在你的耳边悄声说道") then
	     local strong=w[2]
		 self.strong=strong
...
function wudang:gongfu(npc,location,depth)

   if zone_entry(location)==true or string.find(location,"龙王殿") then
	...
	--宋远桥在你的耳边悄声说道：老头子已追查到此人是我武当出身，尤为擅长玄虚刀法的功夫。
	  local l,w=wait.regexp("^(> |)宋远桥在你的耳边悄声说道：据门派弟子来报，此人是来自(.*)的.*，尤为擅长(.*)的功夫。|^(> |)宋远桥在你的耳边悄声说道：老头子已追查到此人是我(.*)出身，尤为擅长(.*)的功夫。",5)
	  if l==nil then
	     self:ask_job()
		 return
	  end
...
function wudang:sure_enemy_skill() --回调函数
end

function wudang:fail(id) --回调函数
...
function wudang:deal_bigword(deep)
...
function wudang:catch_place()

   wait.make(function()
        local l,w=wait.regexp("^(> |)宋远桥在你的耳边悄声说道：据说(.*)正在(.*)周围方圆(.*)里之内捣乱，你马上去给我巡视一圈。$|^(> |)宋远桥在你的耳边悄声说道：据说(.*)正在(.*)捣乱，你马上去给我巡视一圈。$|^(> |)宋远桥说道：「你刚做完(.*)任务，还是先(去|)休息一会吧。」$|^(> |)宋远桥说道：「现在暂时没有适合你的工作。」$|^(> |)宋远桥在你的耳边悄声说道：据说(.*)正在$|^(> |)宋远桥说道：「我不是告诉你了吗，有人在捣乱。你就快去吧！」$|^(> |)宋远桥说道：「.*的正气还不够，我无法放心让你做这个任务！」$",5)
		  if l==nil then
		     self:ask_job()
		     return
		  end
...
function wudang:ask_job()
   ...
   w.user_alias=al
   w.walkover=function()
     world.Send("ask song yuanqiao about job")
	 world.Send("set ask")
...
宋远桥说道：「你刚做完大理送信任务，还是先休息一会吧。」 宋远桥说道：「你刚做完天地会任务，还是先休息一会吧。」]]
     wait.make(function()
	     local l,w=wait.regexp("^(> |)你向宋远桥打听有关『job』的消息。$",5)
		 if l==nil then
		    self:ask_job()
		    return
		 end

		 if string.find(l,"你向宋远桥打听有关") then
		   self:catch_place()
...
function wudang:shield()
end

function wudang:job_failure() --回调函数
end

function wudang:giveup()
 ...
 local w
   ...
   w.walkover=function()
	   ...
	   --宋远桥说道：「叶知秋，你又没在我这里领任务，瞎放弃什么呀！」
	   --宋远桥说道：「叶知秋，你又没在我这里领任务，瞎放弃什么呀！」
	      local l,w=wait.regexp("宋远桥说道：「"..self.playname.."，这个任务确实比较难完成，下次给你简单的，先退下吧！」$|^(> |)宋远桥说道：「"..self.playname.."，你又没在我这里领任务，瞎放弃什么呀！」$|^(> |)宋远桥说道：「"..self.playname.."，你太让我失望了，居然这么点活都干不好，先退下吧！」$",5)
		  if l==nil then
		     self:giveup()
		     return
		  end
...
function wudang:jobDone()--回调
end
--[[
你获得了一百二十七点经验，二十九点潜能！你的侠义正气增加了！
宋远桥为你在钱庄里存入三锭黄金。
]]
function wudang:is_reward()
...
function wudang:reward()
	...
	world.EnableTrigger("robber_place",false)
	world.DeleteTrigger("robber_place")
	local combat_end_time=os.time()
	if combat_start_time~=nil then
	  combat_usedtime = os.difftime (combat_end_time, combat_start_time)
	  --world.AppendToNotepad (WorldName().."_武当任务:",os.date()..": 战斗结束！回程！(战斗耗时:"..combat_usedtime.."秒)\r\n")
	   CallPlugin ("88f53dbf74b5a0ac2fefbf95", "MsgNote", os.date().."武当任务:战斗结束 回程！(战斗耗时:"..combat_usedtime.."秒)", "white", "black") -- yellow on white
	else
	   CallPlugin ("88f53dbf74b5a0ac2fefbf95", "MsgNote", os.date().."武当任务:战斗结束 回程！", "white", "black") -- yellow on white
	   --world.AppendToNotepad (WorldName().."_武当任务:",os.date()..": 战斗结束！回程！\r\n")
	end
   ...
   self:is_reward()
   w.walkover=function()
		world.Send("ask song yuanqiao about 完成")
       ...
       wait.make(function()
	      ...
	      local l,w=wait.regexp("(> |)你获得了(.*)点经验，(.*)点潜能！你的侠义正气增加了！$|^(> |)你向宋远桥打听有关『完成』的消息。$",5)
		  if l==nil then
		    print("超时")
		    shutdown()
		    self:reward()
...
function wudang:liaoshang_fail()
end


function wudang:full()
...
function wudang:buy_zhengqidan()
 local w=walk.new()
 w.walkover=function()
   wait.make(function()
...
function wudang:eat_zhengqidan()
    wait.make(function()
...
function wudang:look_zhengqidan()
--  二十五颗内息丸(Neixi wan)
 wait.make(function()
   world.Send("i")
   world.Send("set look")
...
function wudang:beast_kill_check()
  --看起来盗墓贼想杀死你！
  --print("auto kill 检查")
  wait.make(function()
...
--  毒蟒(Du mang)
function wudang:check_beast_kill_again(id)
   wait.make(function()
...
set 六合劲 静恒]]
 if self.menpai=="姑苏慕容" or self.menpai=="华山派" or self.menpai=="明教" then
    world.Send("set 六合劲 开合")
 elseif self.menpai=="天龙寺" or self.menpai=="少林派" then
    world.Send("set 六合劲 软手")
 elseif self.menpai=="铁掌帮" or self.menpai=="神龙教" or self.menpai=="星宿派" then
    world.Send("set 六合劲 螺旋")
 elseif self.menpai=="大轮寺" or self.menpai=="丐帮" or self.menpai=="武当派" then
   world.Send("set 六合劲 钻翻")
 elseif self.menpai=="峨嵋派" or self.menpai=="嵩山派" or self.menpai=="昆仑派" or self.menpai=="桃花岛" then
   world.Send("set 六合劲 虚灵")
 elseif self.menpai=="古墓派" then
