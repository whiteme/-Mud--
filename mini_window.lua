...
--]]
--我在原有 movewindow基础上 用类封装了，使用起来更加方便
mini_window={
   new=function()
     local mini={}
	 setmetatable(mini,{__index=mini_window})
	 return mini
   end,
}
...
local function make_mousedown_handler (mwi,popup_menu)

  return function (flags, hotspot_id)
    --print(flags,hotspot_id," ")
	local win = mwi.win
	if flags==32 and popup_menu~=nil then  --鼠标右键弹出菜单
	   popup_menu()
	end
...
function mini_window:install (win, default_position, default_flags, nocheck, default_friends,popup_menu)
  ...
  local movewindow_info = {
    ...
    -- mouse handlers
    movewindow_info.mousedown   = make_mousedown_handler   (movewindow_info,popup_menu)
...
function mini_window:add_drag_handler (win, left, top, right, bottom, cursor)
...
function mini_window:save_state (win)
