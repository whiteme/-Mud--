...
--丘处机说道：「光王,你正在任务繁忙中，还是先休息一会吧。」

function chujian:catch_badman()
  local playername=world.GetVariable("player_name") or ""
  wait.make(function()
    local l,w=wait.regexp("^(> |)丘处机对你说道，“"..playername.."，此人乃是金国完颜洪烈的贴身护卫，乔装打扮入我大宋！”$|^(> |)丘处机说道：「"..playername..",你刚结束全真锄奸，还是先休息一会吧。」$|^(> |)丘处机说道：「"..playername..",你正在任务繁忙中，还是先休息一会吧。」$|^(> |)丘处机说道：「"..playername..",你正在执行追杀金国奸细任务中,为啥不赶快去？」$",5)
	if l==nil then
	  self:ask_job()
	  return
	end
	if string.find(l,"此人乃是金国完颜洪烈的贴身护卫") then
	  self:catch_place()
	...
	if string.find(l,"你刚结束全真锄奸") or string.find(l,"为啥不赶快去") then
	  self.fail(101)
	  return
	end
	if  string.find(l,"还是先休息一会吧") then
	  self.fail(102)
	  return
	end
...
function chujian:get_ling(index)

  if index==nil then
      world.Send("get chongyang ling from corpse")
      world.Send("get gold from corpse")
      world.Send("get silver from corpse")

	else
	    world.Send("get chongyang ling from corpse "..index)
	   world.Send("get gold from corpse "..index)
      world.Send("get silver from corpse "..index)

   end
   wait.make(function()
      local l,w=wait.regexp("^(> |)你从.*的尸体身上搜出一块重阳令。$|^(> |)你附近没有这样东西。$|^(> |)你找不到.*这样东西。$",5)
      if l==nil then
	     self:get_ling(index)
	     return
	  end
	  if string.find(l,"搜出一块重阳令") then
         self:reward()
         return
	  end
...
function chujian:danger_skills(name)
   wait.make(function()
      local l,w=wait.regexp("^(> |)"..name.."运起北冥神功口中默念：真气经由灵台，尺泽，曲池，灵海，天宗汇入气海[北溟]，形成一股浩荡之气。$",10)
	  if l==nil then
	     self:danger_skills(name)
	     return
	  end
	  if string.find(l,"运起北冥神功口中默念") then
		 shutdown()
	     self:flee()
	     return
	  end
...
function chujian:reward()
  ...
  w.walkover=function()
   wait.make(function()
    world.Send("give chongyang ling to qiu chuji")
    local playername=world.GetVariable("player_name") or ""
...
--丘处机对光王长叹一声道，“既然完不成也没关系，下次再来吧。”
function chujian:giveup()
 chujian.co=nil
 local w=walk.new()
  w.walkover=function()
   wait.make(function()
	world.Send("drop chongyang ling")
...
-->
function chujian:killSpy(npc)
     print("kill spy "..npc)

	 --tprint(self.ids)
	 print(self.ids[npc])
    if self.ids[npc]~=nil then
	   self:danger_skills(npc)
	   world.Send("kill "..self.ids[npc])
	   self:auto_pfm()
	else
	   print("下一个点，未有匹配")
	   shutdown()
...
function chujian:auto_pfm()

end

function chujian:get_ids(flag)
  if flag==nil then
	    self.ids={}
  ...
  wait.make(function()
	  ...
	  if string.find(l,")") then
		  ...
		  if string.find(name," ") or string.find(name,"「") or string.find(name,"尸体") or string.find(name,"骸骨") or string.find(name,"白银") or string.find(name,"黄金") or string.find(name,"铜钱") then
		     self:get_ids(false)
			 return
		  end
	     ...
	     --print("ceshi")
		  ...
		  self.ids[name]=string.lower(id)
          self:get_ids(false)
          world.Send("look "..string.lower(id))
...
function chujian:check_id()

	local playername=world.GetVariable("player_name") or ""
	wait.make(function()
	  local l,w=wait.regexp("^(> |)这是玩家"..playername.."要追杀的金国奸细!$",5)
	  if l==nil then
	     return
	  end
	  if string.find(l,"要追杀的金国奸细") then
	     local count=GetLinesInBufferCount()
		 local text=GetLineInfo(count-2, 1)

		 text=string.gsub(text,">","")
		 text=Trim(text)
		  print(text," 金国奸细 ")
         local n,l= string.find(text,"%(")
		  local npc=string.sub(text,1,n-1)
